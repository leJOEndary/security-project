...
if(isset($_GET['step'])===true && empty($_GET['step'])===false){
	if(isset($_POST['next'])){
		...
		if(!empty($username)){
			if(strlen($username)>10 or  strlen($username)<6){
				$error='Username must be in range 6-10 characters';
			}
			...
			elseif(preg_match('/\s/',$username)){
				...
				$error="username can't have whitespaces";
			}
			else{$getFromUser->update('users',$user_id,array('username'=>$username));
  ...
  <?php if($_GET['step']=='2'){ ?>
		<div class='lets-wrapper'>
			<div class='step-letsgo'>
				...
				<p>Twitter is a constantly updating stream of the coolest, most important news, media, sports, TV, conversations and more--all tailored just for you.</p>
				<br/>
				<p>
